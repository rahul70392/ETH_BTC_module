...
export const createEthDeposit = function (element, timestamp, settings) {
    ...
    Wallet.findOne({
        ...
        if (web3.eth.getTransactionReceipt(element.hash) == null) {
            ...
            setTimeout(() => {
                module.exports.createEthDeposit(element, timestamp, settings);
            }, 300000);
            return;
        }
        ...
        if (response.role === 'USER') {
                ...
                if (hashPresentEth == null) {
                    ...
                    if (BN(valueSent).gte(settings.minDeposit.ethereum)) {
                        EthDeposit({
                            ...
                            sentTo: element.to,
                            txHash: element.hash,
                            amountActual: valueSent,
                            amountFixed: valueFixed,
            ...
            EthDeposit({
                ...
                if (result) {
                    console.log((new Date().toGMTString()) + ' ==> Admin Ether Deposit: ' + element.hash);
                }
            }).catch((err) => {
                console.log((new Date().toGMTString()) + ' ==> ' + err);
            });
